...
public class PurchasingTest {
    ...
    public void testBuyIllegalArgument() {
        ...
        try {
            purch.buy(part, 0);
            fail("Purchasing.buy() should throw an IllegalArgumentException if the count argument is zero!");
        } catch (IllegalArgumentException e) {
        }
        try {
            purch.buy(part, -1);
            fail("Purchasing.buy() should throw an IllegalArgumentException if the count argument is negative!");
        } catch (IllegalArgumentException e) {
        }
    }
    ...
    public void alarmTest() {
        rStock.register(purch);
        rStock.insert(part, 25);
        rStock.delete(part, 16);
        assertEquals("Purchasing.alarm() should restock if the amount is lower than minStockitems!", 100,
                rStock.get(part));
    }
}
